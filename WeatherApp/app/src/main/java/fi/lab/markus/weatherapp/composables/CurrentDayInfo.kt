...
@Composable
fun CurrentDayInfo(currentTime: Int, weatherResponse: WeatherResponse, userTemp: String) {
        ...
        for (i in currentTime + 1 until currentTime + 13) {
            ...
            item {
                Column(
                    ...
                    Text(text = "$hourTemp $userTemp", fontSize = 20.sp)
                    // Displays a weather code icon based on the weather code for the current hour
                    // and night status
                    WeatherCode(weatherResponse.hourly.weatherCode[i], 75F, isNight)
                }
            }
        }
    }
