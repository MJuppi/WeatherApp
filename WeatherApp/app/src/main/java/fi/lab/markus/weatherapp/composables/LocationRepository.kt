...
class LocationRepository(private val context: Context) {
    /**
     * Function to start location updates by checking permissions and requesting location updates
     * if granted.
     */
    fun startLocationUpdates(callback: (Location?) -> Unit) {
        ...
        if (ContextCompat.checkSelfPermission(
            ...
            val locationRequest =
                LocationRequest.Builder(10000L).setPriority(Priority.PRIORITY_HIGH_ACCURACY)
                    .setIntervalMillis(5000L).build()
